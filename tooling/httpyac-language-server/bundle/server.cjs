    ...
    var AbstractMessageReader = class {
      ...
      asError(error2) {
        if (error2 instanceof Error) {
          ...
          return new Error("Reader received error. Reason: ".concat(Is.string(error2.message) ? error2.message : "unknown"));
        }
      }
    };
        ...
        try {
          ...
          while (true) {
            if (this.nextMessageLength === -1) {
              ...
              if (!contentLength) {
                this.fireError(new Error("Header must provide a Content-Length property.\n".concat(JSON.stringify(Object.fromEntries(headers)))));
                return;
              }
              const length = parseInt(contentLength);
              if (isNaN(length)) {
                this.fireError(new Error("Content-Length value must be a number. Got ".concat(contentLength)));
                return;
              }
    ...
    var AbstractMessageWriter = class {
      ...
      asError(error2) {
        if (error2 instanceof Error) {
          ...
          return new Error("Writer received error. Reason: ".concat(Is.string(error2.message) ? error2.message : "unknown"));
        }
      }
    };
    ...
    function createMessageConnection(messageReader, messageWriter, _logger, options) {
            ...
            if (!handlerResult) {
              ...
              promise.then((resultOrError) => {
                ...
                if (error2 instanceof messages_1.ResponseError) {
                  ...
                  replyError(new messages_1.ResponseError(messages_1.ErrorCodes.InternalError, "Request ".concat(requestMessage.method, " failed with message: ").concat(error2.message)), requestMessage.method, startTime);
                } else {
        ...
        if (responseMessage.id === null) {
          ...
          if (responsePromise !== void 0) {
            ...
            try {
              ...
              if (error2.message) {
                logger.error("Response handler '".concat(responsePromise.method, "' failed with message: ").concat(error2.message));
              } else {
      ...
      function handleNotification(message) {
        ...
        if (notificationHandler || starNotificationHandler) {
          try {
            ...
            if (error2.message) {
              logger.error("Notification handler '".concat(message.method, "' failed with message: ").concat(error2.message));
            } else {
      ...
      function traceReceivedResponse(message, responsePromise) {
        ...
        if (traceFormat === TraceFormat.Text) {
          ...
          if (responsePromise) {
            const error2 = message.error ? " Request failed: ".concat(message.error.message, " (").concat(message.error.code, ").") : "";
            tracer.log("Received response '".concat(responsePromise.method, " - (").concat(message.id, ")' in ").concat(Date.now() - responsePromise.timerStart, "ms.").concat(error2), data);
          } else {
...
var require_main = __commonJS({
  "node_modules/vscode-jsonrpc/lib/node/main.js"(exports2) {
    ...
    function generateRandomPipeName() {
      ...
      let result;
      if (XDG_RUNTIME_DIR) {
        result = path3.join(XDG_RUNTIME_DIR, "vscode-ipc-".concat(randomSuffix, ".sock"));
      } else {
        result = path3.join(os.tmpdir(), "vscode-".concat(randomSuffix, ".sock"));
      }
      ...
      var TextEditChangeImpl = (
        ...
        (function() {
          ...
          TextEditChangeImpl2.prototype.replace = function(range2, newText, annotation) {
            ...
            var id;
            if (annotation === void 0) {
              edit = TextEdit3.replace(range2, newText);
            } else if (ChangeAnnotationIdentifier.is(annotation)) {
              id = annotation;
    ...
    var BulkUnregistrationImpl = class {
      ...
      disposeSingle(arg) {
        ...
        this._connection.sendRequest(vscode_languageserver_protocol_1.UnregistrationRequest.type, params).then(() => {
          ...
          this._connection.console.info("Un-registering request handler for ".concat(unregistration.id, " failed."));
        });
        return true;
      }
    };
    var RemoteClientImpl = class {
      ...
      registerSingle1(unregistration, type, registerOptions) {
        ...
        return this.connection.sendRequest(vscode_languageserver_protocol_1.RegistrationRequest.type, params).then((_result) => {
          ...
          this.connection.console.info("Registering request handler for ".concat(method, " failed."));
        ...
        return this.connection.sendRequest(vscode_languageserver_protocol_1.RegistrationRequest.type, params).then((_result) => {
          return vscode_languageserver_protocol_1.Disposable.create(() => {
            this.unregisterSingle(id, method).catch(() => {
              this.connection.console.info("Un-registering capability with id ".concat(id, " failed."));
            });
          });
        }, (_error) => {
          this.connection.console.info("Registering request handler for ".concat(method, " failed."));
      ...
      unregisterSingle(id, method) {
        ...
        return this.connection.sendRequest(vscode_languageserver_protocol_1.UnregistrationRequest.type, params).catch(() => {
          this.connection.console.info("Un-registering request handler for ".concat(id, " failed."));
        });
      }
    ...
    function patchConsole(logger) {
      ...
      console.assert = function assert(assertion, ...args) {
        ...
        if (args.length === 0) {
          ...
          logger.error("Assertion failed: ".concat(message, " ").concat(serialize(rest)));
        }
      };
      console.count = function count(label = "default") {
        ...
        logger.log("".concat(message, ": ").concat(message));
      };
      ...
      console.error = function error2(...args) {
...
var methodSet = new Set(HTTP_METHODS);
var metadataPattern = /^\s*(?:#|\/\/)\s*@([A-Za-z][\w-]*)(?:(?:\s+|=\s*)(.+?))?\s*$/;
var requestPattern = /^\s*([A-Za-z][A-Za-z-]*)\s+(\S+)(?:\s+HTTP\/\d(?:\.\d)?)?\s*$/;
var variablePattern = /^\s*@?([A-Za-z_][\w.-]*)\s*(=|:=)\s*(.*?)\s*$/;
var regionSeparatorPattern = /^\s*#{3,}(?:\s+.*)?$/;
var pathMetadata = /* @__PURE__ */ new Map([
  ["import", "import"],
  ["proto", "proto"]
]);
function range(line, start, end) {
  return import_vscode_languageserver.Range.create(import_vscode_languageserver.Position.create(line, start), import_vscode_languageserver.Position.create(line, end));
}
function valueRange(line, source, value) {
...
function isPostBodySyntax(source) {
  return regionSeparatorPattern.test(source) || metadataPattern.test(source) || /^\s*\?\?/.test(source) || /^\s*>>!?/.test(source) || /^\s*>\s*(?:\{%|.+\.(?:js|mjs|cjs|ts)\s*$)/i.test(source) || /^\s*HTTP\/\d(?:\.\d)?\s+\d{3}\b/i.test(source) || /^\s*\{\{\s*[@+](?:request|response|streaming)\b/i.test(source);
}
function isHttpYacLineComment(source) {
  return Boolean(/^\s*\/\/\s*(.*)\s*$/.exec(source)?.[1]);
}
function findHttpYacBlockCommentEnd(lines, startLine, endLine = lines.length - 1) {
  if (!/^\s*\/\*$/.test(lines[startLine])) return void 0;
  for (let line = startLine + 1; line <= endLine; line++) {
    if (/^\s*\*\/\s*$/.test(lines[line])) return line;
  }
  return void 0;
}
function findJsonContentStartLine(lines, bodyStart, requestEnd) {
...
function findJsonBodyEndLine(lines, bodyStart, requestEnd) {
  const stack = [];
  let started = false;
  let inString = false;
  let escaped = false;
...
function normalizeJsonBodyComments(text) {
  ...
  for (let line = 0; line < lines.length; line++) {
    ...
    if (closingLine === void 0) continue;
...
function addPath(paths, kind, line, source, rawPath) {
  const path3 = unquote(rawPath.replace(/^\s*@/, "").trim());
  if (!path3 || path3.includes("{{")) return;
  paths.push({ kind, path: path3, range: valueRange(line, source, path3) });
}
function indexDocument(uri, text) {
  ...
  for (let lineNumber = 0; lineNumber < lines.length; lineNumber++) {
    ...
    if (metadataMatch) {
      ...
      const entry = {
        ...
        value,
        valueRange: value ? valueRange(lineNumber, source, value) : void 0
      };
      metadata.push(entry);
    ...
    if (requestMatch && (methodSet.has(requestMatch[1].toUpperCase()) || /^[A-Z-]+$/.test(requestMatch[1]))) {
      ...
      currentRequest = {
        name: pendingName?.value ?? "".concat(method.toUpperCase(), " ").concat(url),
        explicitName: Boolean(pendingName?.value),
        responseVariableName: pendingName?.value ? responseVariableName(pendingName.value) : void 0,
  ...
  for (const request of requests) {
    ...
    const hasJsonContentType = lines.slice(request.startLine + 1, bodyStart).some((line) => /^\s*Content-Type\s*:\s*application\/(?:[\w.+-]*\+)?json\b/i.test(line));
    const jsonContentStart = hasJsonContentType ? findJsonContentStartLine(lines, bodyStart, request.endLine) : void 0;
    const hasJsonBody = jsonContentStart !== void 0 && /^\s*[\[{]/.test(lines[jsonContentStart]);
    let bodyEnd = hasJsonBody ? findJsonBodyEndLine(lines, bodyStart, request.endLine) : request.endLine;
    ...
    jsonBodies.push({
      range: import_vscode_languageserver.Range.create(
        import_vscode_languageserver.Position.create(bodyStart, 0),
        import_vscode_languageserver.Position.create(bodyEnd, lines[bodyEnd]?.length ?? 0)
      ),
...
function responseVariableName(name) {
  return name.trim().replace(/\s/g, "-").replace(/-./g, (value) => value[1].toUpperCase());
}
function wordAt(index, position) {
  const candidates = [
    ...index.requests,
  ...
  return candidates.find(
    ({ range: candidate }) => candidate.start.line === position.line && candidate.start.character <= position.character && candidate.end.character >= position.character
  );
}
...
var HTTP_EXTENSIONS = /* @__PURE__ */ new Set([".http", ".rest"]);
function uriToPath(uri) {
  try {
    return uri.startsWith("file:") ? (0, import_node_url.fileURLToPath)(uri) : void 0;
  } catch {
    return void 0;
  }
}
function resolvePath(uri, reference) {
  const fileName = uriToPath(uri);
  if (!fileName || /^(?:https?|wss?|grpc):\/\//i.test(reference)) return void 0;
  const normalized = reference.replace(/^~(?=[\\/])/, process.env.USERPROFILE ?? "~");
  return import_node_path.default.resolve(import_node_path.default.dirname(fileName), normalized);
}
var WorkspaceIndex = class {
  ...
  async loadFile(fileName) {
    const uri = (0, import_node_url.pathToFileURL)(fileName).toString();
    const existing = this.documents.get(uri);
    if (existing) return existing;
    try {
      const text = await import_node_fs.promises.readFile(fileName, "utf8");
      return this.update(uri, text);
    } catch {
      return void 0;
    }
  }
  async loadImports(index, seen = /* @__PURE__ */ new Set()) {
  ...
  async scanDirectory(directory) {
    ...
    for (const entry of entries) {
      ...
      const fileName = import_node_path.default.join(directory, entry.name);
      if (entry.isDirectory()) await this.scanDirectory(fileName);
      else if (HTTP_EXTENSIONS.has(import_node_path.default.extname(entry.name).toLowerCase())) await this.loadFile(fileName);
    }
  }
  async reachable(index) {
  ...
  async requestDefinitions(index, name) {
    const documents2 = await this.reachable(index);
    return documents2.flatMap(
      (document) => document.requests.filter((request) => request.name === name).map((request) => ({ uri: document.uri, range: request.range }))
    );
  }
  async variableDefinitions(index, name) {
    const documents2 = await this.reachable(index);
    const rootName = name.split(".")[0];
    return documents2.flatMap((document) => [
      ...document.variables.filter((variable) => variable.name === name || variable.name === rootName).map((variable) => ({ uri: document.uri, range: variable.range })),
      ...document.requests.filter((request) => request.responseVariableName === rootName).map((request) => ({ uri: document.uri, range: request.range }))
    ]);
  }
  requestOccurrences(name) {
    return this.all().flatMap((document) => [
      ...document.requests.filter((request) => request.name === name).map((occurrence) => ({ uri: document.uri, occurrence })),
      ...document.requestReferences.filter((reference) => reference.name === name).map((occurrence) => ({ uri: document.uri, occurrence }))
    ]);
  }
  variableOccurrences(name) {
    return this.all().flatMap((document) => [
      ...document.variables.filter((variable) => variable.name === name).map((occurrence) => ({ uri: document.uri, occurrence })),
      ...document.variableReferences.filter(
        (reference) => reference.name === name || reference.name.startsWith("".concat(name, "."))
      ).map((reference) => ({
        uri: document.uri,
        occurrence: {
          name,
  ...
  responseVariableOccurrences(name) {
    return this.all().flatMap(
      (document) => document.variableReferences.filter((reference) => reference.rootName === name).map((reference) => ({
        uri: document.uri,
  ...
  hasRequest(name) {
    return this.all().some(
      (document) => document.requests.some((request) => request.name === name)
    );
  }
  requestNameForResponseVariable(variableName) {
    return this.all().flatMap((document) => document.requests).find((request) => request.responseVariableName === variableName)?.name;
  }
  responseVariableForRequest(name) {
    return this.all().flatMap((document) => document.requests).find((request) => request.name === name)?.responseVariableName;
  }
  findPath(index, line, character) {
    return index.paths.find(
      ({ range: range2 }) => range2.start.line === line && range2.start.character <= character && range2.end.character >= character
    );
  }
};
...
function linePrefix(index, position) {
...
function staticItems(entries, kind) {
...
async function completions(workspace2, index, position, catalog2) {
  const prefix = linePrefix(index, position);
  const reachable = await workspace2.reachable(index);
  const requestAtLine = index.requests.find(
    (request) => request.startLine < position.line && request.endLine >= position.line
  );
  if (/^\s*#\s*@(?:forceRef|ref)\s+\S*$/i.test(prefix)) {
    return dedupe(reachable.flatMap((document) => document.requests).filter((request) => request.explicitName).map((request) => ({
      label: request.name,
      kind: import_vscode_languageserver2.CompletionItemKind.Reference,
      detail: "".concat(request.method ?? "REQUEST", " ").concat(request.url ?? "").trim(),
  ...
  if (/\{\{\s*[\w.$-]*$/.test(prefix)) {
    const localItems = [
      ...reachable.flatMap((document) => document.variables).map((variable) => ({
        label: variable.name,
        kind: import_vscode_languageserver2.CompletionItemKind.Variable,
        detail: "httpYac file variable"
      })),
      ...reachable.flatMap((document) => document.requests).filter((request) => request.explicitName && request.responseVariableName).map((request) => ({
        label: request.responseVariableName ?? request.name,
        kind: import_vscode_languageserver2.CompletionItemKind.Variable,
        detail: "Named response"
      }))
    ];
    return dedupe([...localItems, ...staticItems(catalog2.variables, import_vscode_languageserver2.CompletionItemKind.Variable)]);
  }
  if (/^\s*#\s*@[\w-]*$/.test(prefix)) {
    return dedupe(catalog2.metadata.map((entry) => ({
      label: "@".concat(entry.name),
      kind: import_vscode_languageserver2.CompletionItemKind.Property,
      detail: entry.description,
      documentation: entry.description,
      insertText: "@".concat(entry.name).concat(entry.completions?.length ? " " : "")
    })));
  }
  ...
  if (!requestAtLine && /^\s*[A-Za-z-]*$/.test(prefix)) {
    return [
      ...HTTP_METHODS.map((method) => ({
        ...
        insertText: "".concat(method, " ")
      })),
      ...staticItems(catalog2.emptyLine, import_vscode_languageserver2.CompletionItemKind.Snippet)
    ];
  }
  if (requestAtLine && (/^\s*[\w-]*$/.test(prefix) || /^\s*[\w-]+\s*:\s*.*$/.test(prefix))) {
    return dedupe([
      ...knownHeaders.map((header) => ({
        label: header,
        kind: import_vscode_languageserver2.CompletionItemKind.Field,
        insertText: "".concat(header, ": ")
      })),
      ...staticItems(catalog2.headers, import_vscode_languageserver2.CompletionItemKind.Field)
    ]);
  }
  return [];
}
async function pathCompletions(index, prefix) {
  const uriPath = resolvePath(index.uri, ".");
  if (!uriPath) return [];
  const typed = prefix.match(/(?:\s|<)([^\s]*)$/)?.[1] ?? "";
  const directoryPart = typed.replace(/[\\/][^\\/]*$/, "") || ".";
  const base = import_node_path2.default.resolve(uriPath, directoryPart);
  try {
    const entries = await import_node_fs2.promises.readdir(base, { withFileTypes: true });
    return entries.map((entry) => ({
      label: entry.name + (entry.isDirectory() ? import_node_path2.default.sep : ""),
      kind: entry.isDirectory() ? import_vscode_languageserver2.CompletionItemKind.Folder : import_vscode_languageserver2.CompletionItemKind.File
    }));
  } catch {
    return [];
  }
}
async function diagnostics(workspace2, index, knownMetadata2, knownVariables2 = /* @__PURE__ */ new Set()) {
  const result = [];
  const reachable = await workspace2.reachable(index);
  const allNamedRequests = reachable.flatMap((document) => document.requests).filter((request) => request.explicitName);
  for (const request of index.requests.filter((item) => item.explicitName)) {
    if (!isValidRequestName(request.name)) {
      result.push(error(request.range, "\u975E\u6CD5\u7684 request name: ".concat(request.name), "invalid-name"));
    }
    if (allNamedRequests.filter((item) => item.name === request.name).length > 1) {
      result.push(error(
        request.range,
        "\u5F53\u524D\u6587\u4EF6\u6216\u5BFC\u5165\u56FE\u4E2D\u5B58\u5728\u91CD\u590D\u7684 request name: ".concat(request.name),
        "duplicate-name"
      ));
    }
  }
  for (const reference of index.requestReferences) {
    ...
    if ((await workspace2.requestDefinitions(index, reference.name)).length === 0) {
      result.push(error(reference.range, "\u672A\u5B9A\u4E49\u7684 request reference: ".concat(reference.name), "undefined-ref"));
    }
  }
  for (const reference of index.variableReferences) {
    const rootName = reference.rootName;
    if (KNOWN_BUILTINS.has(rootName) || knownVariables2.has(reference.name) || knownVariables2.has(rootName)) {
      continue;
    }
    if ((await workspace2.variableDefinitions(index, reference.name)).length === 0) {
      result.push(warning(
        reference.rootRange,
        "\u672A\u5B9A\u4E49\u7684\u53D8\u91CF: ".concat(reference.rootName),
        "undefined-variable"
      ));
    }
  }
  for (const request of index.requests) {
    if (request.method && !HTTP_METHODS.includes(request.method.toUpperCase())) {
      result.push(error(request.methodRange ?? request.range, "\u4E0D\u652F\u6301\u7684\u65B9\u6CD5: ".concat(request.method), "invalid-method"));
    }
    if (request.url && !isValidUrl(request.url)) {
      result.push(warning(request.urlRange ?? request.range, "URL \u770B\u8D77\u6765\u4E0D\u5B8C\u6574: ".concat(request.url), "invalid-url"));
    }
  }
  for (const item of index.metadata) {
    if (!knownMetadata2.has(item.name.toLowerCase())) {
      result.push(warning(item.range, "\u672A\u77E5\u7684 httpYac metadata: @".concat(item.name), "unknown-metadata"));
    }
    if (["name", "ref", "forceref", "import"].includes(item.name.toLowerCase()) && !item.value) {
      result.push(error(item.range, "@".concat(item.name, " \u9700\u8981\u53C2\u6570"), "missing-metadata-value"));
    }
  ...
  for (const reference of index.paths) {
    ...
    if (!fileName) continue;
    try {
      await import_node_fs2.promises.access(fileName);
    } catch {
      result.push(error(reference.range, "\u6587\u4EF6\u4E0D\u5B58\u5728: ".concat(reference.path), "missing-file"));
    }
  }
  result.push(...await cycleDiagnostics(workspace2, index));
  result.push(...await referenceCycleDiagnostics(workspace2, index));
  for (const body of index.jsonBodies) {
    if (body.text.includes("{{")) continue;
    try {
      JSON.parse(body.text);
    } catch (cause) {
      result.push(error(body.range, "JSON body \u65E0\u6548: ".concat(cause.message), "invalid-json"));
    }
  }
  return result;
}
function validateMetadataParameter(name, value) {
  if (!value) return void 0;
  switch (name.toLowerCase()) {
    case "name":
      return isValidRequestName(value) ? void 0 : "@name \u53C2\u6570\u5305\u542B\u975E\u6CD5\u5B57\u7B26";
    case "ref":
    case "forceref":
      return isValidRequestName(value) || value.includes("{{") ? void 0 : "@".concat(name, " \u53C2\u6570\u5305\u542B\u975E\u6CD5\u5B57\u7B26");
    case "timeout":
      return /^\s*\d+(?:\.\d+)?(?:\s*ms)?\s*$/i.test(value) || value.includes("{{") ? void 0 : "@timeout \u53C2\u6570\u5FC5\u987B\u4EE5\u6BEB\u79D2\u6570\u5F00\u5934";
    case "loop":
      return /^\s*(?:for\s+(?:\d+|.+\s+of\s+.+)|while\s+.+)\s*$/i.test(value) ? void 0 : "@loop \u53C2\u6570\u5E94\u4E3A `for N`\u3001`for value of iterable` \u6216 `while expression`";
    case "ratelimit":
      return /^\s*(?:(?:slot\s*:?\s*\S+)\s*)?(?:(?:minIdleTime\s*:?\s*\d+)\s*)?(?:max\s*:?\s*\d+)(?:\s+expire\s*:?\s*\d+)?\s*$/i.test(value) ? void 0 : "@ratelimit \u53C2\u6570\u683C\u5F0F\u65E0\u6548";
    default:
      return void 0;
  }
}
function isValidRequestName(name) {
  return /^[^\s#][^#\r\n]*$/.test(name.trim());
}
async function cycleDiagnostics(workspace2, start) {
  ...
  const visit = async (index) => {
    ...
    for (const reference of index.paths.filter((item) => item.kind === "import")) {
      const fileName = resolvePath(index.uri, reference.path);
      if (!fileName) continue;
      const uri = (0, import_node_url2.pathToFileURL)(fileName).toString();
      if (stack.has(uri)) {
        result.push(error(reference.range, "\u68C0\u6D4B\u5230 import \u5FAA\u73AF: ".concat(reference.path), "import-cycle"));
        continue;
      }
      const imported = await workspace2.loadFile(fileName);
      if (imported) await visit(imported);
    }
...
async function referenceCycleDiagnostics(workspace2, start) {
  const documents2 = await workspace2.reachable(start);
  const requests = documents2.flatMap((document) => document.requests);
  const byName = new Map(
    requests.filter((request) => request.explicitName).map((request) => [request.name, request])
  );
  const result = [];
  const visited = /* @__PURE__ */ new Set();
  const stack = /* @__PURE__ */ new Set();
  const visit = (name) => {
    ...
    for (const reference of request.references) {
      if (stack.has(reference.name)) {
        result.push(error(
          reference.range,
          "\u68C0\u6D4B\u5230 request reference \u5FAA\u73AF: ".concat(reference.name),
          "reference-cycle"
        ));
      } else {
...
function error(range2, message, code) {
  return { range: range2, message, code, source: "httpyac", severity: import_vscode_languageserver2.DiagnosticSeverity.Error };
}
function warning(range2, message, code) {
  return { range: range2, message, code, source: "httpyac", severity: import_vscode_languageserver2.DiagnosticSeverity.Warning };
}
async function definition(workspace2, index, position) {
  const pathReference = workspace2.findPath(index, position.line, position.character);
  if (pathReference) {
    const fileName = resolvePath(index.uri, pathReference.path);
    return fileName ? [import_vscode_languageserver2.Location.create((0, import_node_url2.pathToFileURL)(fileName).toString(), import_vscode_languageserver2.Range.create(0, 0, 0, 0))] : [];
  }
  const occurrence = wordAt(index, position);
  if (!occurrence) return [];
  if (index.requestReferences.includes(occurrence)) {
    return workspace2.requestDefinitions(index, occurrence.name);
  }
  const variableReference = index.variableReferences.find((item) => item === occurrence);
  const responseRequestName = variableReference ? workspace2.requestNameForResponseVariable(variableReference.rootName) : void 0;
  if (responseRequestName) {
    return workspace2.requestDefinitions(index, responseRequestName);
  }
  return workspace2.variableDefinitions(index, occurrence.name);
}
function references(workspace2, index, position) {
  const occurrence = wordAt(index, position);
  if (!occurrence) return [];
  const variableReference = index.variableReferences.find((item) => item === occurrence);
  const requestName = index.requests.includes(occurrence) || index.requestReferences.includes(occurrence) ? occurrence.name : variableReference ? workspace2.requestNameForResponseVariable(variableReference.rootName) : void 0;
  const responseVariable = requestName ? workspace2.responseVariableForRequest(requestName) : void 0;
  const matches = requestName ? [
    ...workspace2.requestOccurrences(requestName),
    ...responseVariable ? workspace2.responseVariableOccurrences(responseVariable) : []
  ] : workspace2.variableOccurrences(
    variableReference?.rootName ?? occurrence.name
  );
  return matches.map((item) => import_vscode_languageserver2.Location.create(item.uri, item.occurrence.range));
}
function rename(workspace2, index, position, newName) {
  const occurrence = wordAt(index, position);
  ...
  const changes = {};
  if (requestName) {
    for (const item of workspace2.requestOccurrences(requestName)) {
      (changes[item.uri] ??= []).push(import_vscode_languageserver2.TextEdit.replace(item.occurrence.range, newName));
    }
    const responseVariable = workspace2.responseVariableForRequest(requestName);
    if (responseVariable) {
      const newResponseVariable = responseVariableName(newName);
      for (const item of workspace2.responseVariableOccurrences(responseVariable)) {
        (changes[item.uri] ??= []).push(
          import_vscode_languageserver2.TextEdit.replace(item.occurrence.range, newResponseVariable)
        );
      }
    }
  } else {
    const variableName = variableReference?.rootName ?? occurrence.name;
    for (const item of workspace2.variableOccurrences(variableName)) {
      (changes[item.uri] ??= []).push(import_vscode_languageserver2.TextEdit.replace(item.occurrence.range, newName));
    }
  }
  return { changes };
}
function documentLinks(index) {
  return index.paths.flatMap((reference) => {
    const fileName = resolvePath(index.uri, reference.path);
    return fileName ? [{ range: reference.range, target: (0, import_node_url2.pathToFileURL)(fileName).toString(), tooltip: "".concat(reference.kind, " file") }] : [];
  });
}
function formattingEdits(index) {
  const edits = [];
  for (let line = 0; line < index.lines.length; line++) {
    ...
    if (trimmed.length !== source.length) {
      edits.push(import_vscode_languageserver2.TextEdit.del({
...
function workspaceSymbols(workspace2, query) {
  const lowerQuery = query.toLowerCase();
  return workspace2.all().flatMap(
    (index) => index.requests.filter(
      (request) => request.explicitName && request.name.toLowerCase().includes(lowerQuery)
    ).map((request) => import_vscode_languageserver2.SymbolInformation.create(
      request.name,
      import_vscode_languageserver2.SymbolKind.Method,
      request.range,
      index.uri,
      "".concat(request.method ?? "", " ").concat(request.url ?? "").trim()
    ))
  );
}
function hover(index, position, catalog2) {
  const metadata = index.metadata.find(
    (item) => item.range.start.line === position.line && item.range.start.character <= position.character && item.range.end.character >= position.character
  );
  if (metadata) {
    const known = catalog2.metadata.find((item) => item.name.toLowerCase() === metadata.name.toLowerCase());
    return known ? { contents: { kind: import_vscode_languageserver2.MarkupKind.Markdown, value: "**@".concat(known.name, "**\n\n").concat(known.description ?? "httpYac metadata") } } : null;
  }
  const request = index.requests.find(
    (item) => item.startLine === position.line && item.methodRange && item.methodRange.start.character <= position.character && item.methodRange.end.character >= position.character
  );
  if (request) {
    return {
      contents: {
        kind: import_vscode_languageserver2.MarkupKind.Markdown,
        value: "**".concat(request.method, "** request\n\n").concat(request.description ?? request.url ?? "httpYac request")
      }
    };
  }
  const variable = index.variableReferences.find(
    ({ range: range2 }) => range2.start.line === position.line && range2.start.character <= position.character && range2.end.character >= position.character
  );
  if (variable) {
    const known = catalog2.variables.find(
      (item) => normalizeCatalogVariable(item.name) === variable.name || normalizeCatalogVariable(item.name) === variable.rootName
    );
    const request2 = index.requests.find(
      (item) => item.responseVariableName === variable.rootName
    );
    const definition2 = index.variables.find(
      (item) => variable.name === item.name || variable.name.startsWith("".concat(item.name, "."))
    );
    const description = known?.description ?? (request2 ? "Named response from ".concat(request2.method ?? "request", " ").concat(request2.url ?? "").trim() : definition2 ? "httpYac file variable" : void 0);
    return description ? {
      contents: {
        kind: import_vscode_languageserver2.MarkupKind.Markdown,
  ...
  if (header) {
    const known = catalog2.headers.find(
      (item) => item.name.toLowerCase() === header.toLowerCase()
    );
    return {
      contents: {
        kind: import_vscode_languageserver2.MarkupKind.Markdown,
        value: "**".concat(header, "** HTTP header\n\n").concat(known?.description ?? "Header names are case-insensitive.")
      }
    };
  }
  return null;
}
function catalogVariableNames(catalog2) {
  return new Set(catalog2.variables.map((item) => normalizeCatalogVariable(item.name)));
}
function normalizeCatalogVariable(name) {
  return name.trim().split(/\s/)[0].replace(/\(\)$/, "");
}
function codeActions(index, diagnostics2, requestedRange) {
  const actions = [];
  for (const diagnostic of diagnostics2) {
    if (diagnostic.code === "undefined-ref") {
      ...
      const similar = nearest(
        name,
        index.requests.filter((request2) => request2.explicitName).map((request2) => request2.name)
      );
      if (similar) {
        actions.push({
          title: "\u6539\u4E3A @ref ".concat(similar),
          kind: import_vscode_languageserver2.CodeActionKind.QuickFix,
          diagnostics: [diagnostic],
          edit: { changes: { [index.uri]: [import_vscode_languageserver2.TextEdit.replace(diagnostic.range, similar)] } }
        });
      }
    }
  ...
  const request = index.requests.find(
    (item) => !item.explicitName && requestedRange && item.startLine <= requestedRange.start.line && item.endLine >= requestedRange.end.line
  ...
  if (request && !request.explicitName) {
    actions.push({
      ...
      edit: {
        changes: {
          [index.uri]: [import_vscode_languageserver2.TextEdit.insert(import_vscode_languageserver2.Position.create(request.startLine, 0), "# @name requestName\n")]
        }
      }
    });
  }
  const position = import_vscode_languageserver2.Position.create(0, 0);
  const headerRequest = index.requests.find(
    (item) => requestedRange && item.startLine <= requestedRange.start.line && item.endLine >= requestedRange.end.line
  ) ?? index.requests[0];
  const requestLines = headerRequest ? index.lines.slice(headerRequest.startLine + 1, headerRequest.endLine + 1) : index.lines;
  if (!requestLines.some((line) => /^\s*Content-Type\s*:/i.test(line))) {
    actions.push({
      ...
      edit: {
        changes: {
          [index.uri]: [import_vscode_languageserver2.TextEdit.insert(
            headerRequest ? import_vscode_languageserver2.Position.create(headerRequest.startLine + 1, 0) : position,
...
function nearest(source, candidates) {
  if (!source) return void 0;
  const scored = candidates.filter((candidate) => candidate !== source).map((candidate) => ({ candidate, score: distance(source, candidate) })).sort((left, right) => left.score - right.score);
  return scored[0]?.score <= Math.max(2, Math.floor(source.length / 3)) ? scored[0].candidate : void 0;
}
...
var catalog_default = {
  ...
  headers: [
    ...
    {
      name: "Accept",
      description: "Media type(s)\xA0that is/are acceptable for the response. See\xA0Content negotiation."
    },
    {
      name: "Accept-Charset",
    ...
    },
    {
      ...
      description: "List of acceptable encodings. See\xA0HTTP compression."
    },
    {
      name: "Accept-Language",
      description: "List of acceptable human languages for response. See\xA0Content negotiation."
    },
    {
      name: "Access-Control-Request-Method",
      description: "Initiates a request for\xA0cross-origin resource sharing\xA0with\xA0Origin\xA0(below)."
    },
    {
    ...
    {
      name: "Authorization",
      description: "Authentication credentials for\xA0HTTP authentication."
    },
    {
      name: "Cache-Control",
      description: "Used to specify directives that\xA0must\xA0be obeyed by all caching mechanisms along the request-response chain."
    },
    {
      name: "Connection",
    ...
    },
    {
      ...
      description: "The type of encoding used on the data. See\xA0HTTP compression."
    },
    {
      name: "Content-Length",
      description: "The length of the request body in\xA0octets\xA0(8-bit bytes)."
    },
    {
      name: "Content-MD5",
      description: "A\xA0Base64-encoded binary\xA0MD5\xA0sum of the content of the request body."
    },
    {
      name: "Content-Type",
      description: "The\xA0Media type\xA0of the body of the request (used with POST and PUT requests)."
    },
    {
      name: "Cookie",
      description: "An\xA0HTTP cookie\xA0previously sent by the server with\xA0Set-Cookie\xA0(below)."
    },
    {
      name: "Date",
      description: 'The date and time at which the message was originated (in "HTTP-date" format as defined by\xA0RFC 7231 Date/Time Formats).'
    },
    {
      name: "Expect",
    ...
    },
    {
      ...
      description: "Allows a\xA0304 Not Modified\xA0to be returned if content is unchanged."
    },
    {
      name: "If-None-Match",
      description: "Allows a\xA0304 Not Modified\xA0to be returned if content is unchanged, see\xA0HTTP ETag."
    },
    {
      name: "If-Range",
    ...
    {
      name: "Origin",
      description: "Initiates a request for\xA0cross-origin resource sharing\xA0(asks server for\xA0Access-Control-*\xA0response fields)."
    },
    {
      name: "Pragma",
    ...
    {
      name: "Range",
      description: "Request only part of an entity. Bytes are numbered from 0. See\xA0Byte serving."
    },
    {
      name: "Referer",
    ...
    {
      name: "TE",
      description: 'The transfer encodings the user agent is willing to accept: the same values as for the response header field Transfer-Encoding can be used, plus the "trailers" value (related to the "chunked" transfer method) to notify the server it expects to receive additional fields in the trailer after the last, zero-sized, chunk. Only\xA0trailers\xA0is supported in HTTP/2.'
    },
    {
      name: "Trailer",
      description: "The Trailer general field value indicates that the given set of header fields is present in the trailer of a message encoded with\xA0chunked transfer coding."
    },
    {
      name: "Transfer-Encoding",
      description: "The form of encoding used to safely transfer the entity to the user.\xA0Currently defined methods\xA0are:\xA0chunked, compress, deflate, gzip, identity. Must not be used with HTTP/2"
    },
    {
      name: "User-Agent",
      description: "The\xA0user agent string\xA0of the user agent."
    },
    {
      name: "Upgrade",
    ...
    },
    {
      ...
      description: "Tells a server which (presumably in the middle of a HTTP -> HTTPS migration) hosts mixed content that the client would prefer redirection to HTTPS and can handle\xA0Content-Security-Policy: upgrade-insecure-requests Must not be used with HTTP/2"
    },
    {
      name: "X-Requested-With",
      description: "Mainly used to identify\xA0Ajax\xA0requests (most\xA0JavaScript frameworks\xA0send this field with value of\xA0XMLHttpRequest); also identifies Android apps using WebView"
    },
    {
      name: "DNT",
      description: "Requests a web application to disable their tracking of a user. This is Mozilla`s version of the X-Do-Not-Track header field (since\xA0Firefox 4.0\xA0Beta 11).\xA0Safari\xA0and\xA0IE9\xA0also have support for this field.\xA0On March 7, 2011, a draft proposal was submitted to IETF.\xA0The\xA0W3C\xA0Tracking Protection Working Group is producing a specification."
    },
    {
      name: "X-Forwarded-For",
      description: "A\xA0de facto\xA0standard\xA0for identifying the originating IP address of a client connecting to a web server through an HTTP proxy or load balancer. Superseded by\xA0Forwarded\xA0header."
    },
    {
      name: "X-Forwarded-Host",
      description: "A\xA0de facto\xA0standard\xA0for identifying the original host requested by the client in the\xA0Host\xA0HTTP request header, since the host name and/or port of the reverse proxy (load balancer) may differ from the origin server handling the request. Superseded by\xA0Forwarded\xA0header."
    },
    {
      name: "X-Forwarded-Proto",
      description: "A\xA0de facto\xA0standard\xA0for identifying the originating protocol of an HTTP request, since a reverse proxy (or a load balancer) may communicate with a web server using HTTP even if the request to the reverse proxy is HTTPS. An alternative form of the header (X-ProxyUser-Ip) is used by Google clients talking to Google servers. Superseded by\xA0Forwarded\xA0header."
    },
    {
    ...
    {
      name: "X-UIDH",
      description: 'Server-side\xA0deep packet insertion\xA0of a unique ID identifying customers of\xA0Verizon Wireless; also known as "perma-cookie" or "supercookie"'
    },
    {
      name: "X-Csrf-Token",
      description: "Used to prevent\xA0cross-site request forgery. Alternative header names are:\xA0X-CSRFToken\xA0and\xA0X-XSRF-TOKEN"
    },
    {
      name: "X-Request-ID",
...
// src/security.ts
function loadSafeHttpYacCatalog() {
  delete process.env.HTTPYAC_PLUGIN;
  return structuredClone(catalog_default);
}
...
var catalog = loadSafeHttpYacCatalog();
var knownMetadata = new Set(catalog.metadata.map((item) => item.name.toLowerCase()));
var knownVariables = catalogVariableNames(catalog);
connection.onInitialize((params) => {
  const roots = params.workspaceFolders?.map((folder) => folder.uri) ?? (params.rootUri ? [params.rootUri] : []);
...
connection.onInitialized(() => {
  void connection.client.register(import_node.DidChangeConfigurationNotification.type);
});
...
connection.onPrepareRename((params) => {
  ...
  const variableReference = index.variableReferences.find(
    (item) => item.rootRange.start.line === params.position.line && item.rootRange.start.character <= params.position.character && item.rootRange.end.character >= params.position.character
  );
  ...
  const occurrence = [
    ...
    (item) => item.range.start.line === params.position.line && item.range.start.character <= params.position.character && item.range.end.character >= params.position.character
  );
...
connection.onWorkspaceSymbol(
  (params) => workspaceSymbols(workspace, params.query)
);
...
function getIndex(uri) {
  ...
  return document ? workspace.update(uri, document.getText()) : void 0;
}
