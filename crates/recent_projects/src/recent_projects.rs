...
pub fn init(cx: &mut App) {
    ...
    cx.on_action(|open_recent_remote: &workspace::welcome::OpenRecentRemoteProject, cx| {
        ...
        with_active_or_new_workspace(cx, move |workspace, window, cx| {
            let replace_current_window = workspace.can_replace_with_project(cx);
            let requesting_window = if create_new_window && !replace_current_window {
                None
            } else {
                window.window_handle().downcast::<MultiWorkspace>()
            };
...
impl RecentProjects {
    ...
    pub fn popover(
        ...
        cx.new(|cx| {
            ...
            let list = Self::new(delegate, fs, 28., window, cx);
