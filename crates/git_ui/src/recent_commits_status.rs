...
use ui::{
    ButtonCommon as _, Clickable as _, Disableable as _, FixedWidth as _, LabelCommon as _,
    SelectableButton as _, StyledExt as _, WithScrollbar as _,
};
...
impl gpui::Render for RecentCommitsStatus {
    fn render(
        ...
        let subject = history
            ...
            .to_owned();
        let short_sha = history.read(cx).head_sha.display_short();
        let font_size = ui::TextSize::Small.rems(cx).to_pixels(window.rem_size());
        let measure_text = |text: &str, font: gpui::Font| {
            window
                .text_system()
                .shape_line(
                    text.to_owned().into(),
        ...
        let gap = gpui::rems(0.25);
        // Give the percentage-width trigger a definite basis instead of relying on intrinsic sizing.
        let entry_width = measure_text(&subject, theme_settings.ui_font(cx).clone())
            + measure_text(&short_sha, theme_settings.buffer_font(cx).clone())
            + icon_size.rems().to_pixels(window.rem_size())
            + gap.to_pixels(window.rem_size()) * 2.
            + ui::DynamicSpacing::Base04
                .rems(cx)
        ...
        gpui::div()
            ...
            .debug_selector(|| "RECENT_COMMITS_ENTRY".into())
            .w(entry_width.min(gpui::rems(48.).to_pixels(window.rem_size())))
            .max_w(gpui::rems(48.))
            .min_w_0()
            .child(
                ui::PopoverMenu::new("git-recent-commits")
                    ...
                    // PopoverMenu applies this to both the anchor position and the anchored element.
                    .offset(gpui::point(gpui::px(0.), gpui::px(2.)))
                    .full_width(true)
                    .trigger(
                        ui::ButtonLike::new("git-last-commit")
                            .aria_label(format!("Recent commits: {short_sha} {subject}"))
                                ...
                                ui::h_flex()
                                    ...
                                    .child(
                                        gpui::div().flex_none().child(
                                            ui::Icon::new(ui::IconName::GitCommit)
                                                .size(icon_size)
                                    ...
                                    .child(
                                        gpui::div()
                                            .debug_selector(|| "RECENT_COMMITS_SHA".into())
                                            .flex_none()
                                            .child(
                                                ui::Label::new(short_sha)
                                    ...
                                    .child(
                                        gpui::div()
                                            .debug_selector(|| "RECENT_COMMITS_SUBJECT".into())
                                            .min_w_0()
                                            .child(
                                                ui::Label::new(subject)
                                                    .size(ui::LabelSize::Small)
                                                    .color(ui::Color::Muted)
                                                    .truncate(),
                                            ),
...
mod tests {
    ...
    struct TestTitleBar {
        status: gpui::Entity<super::RecentCommitsStatus>,
    }
        ...
        fn render(
            ...
            ui::h_flex()
                ...
                .child(
                    ui::h_flex()
                        .min_w_0()
                        .child(ui::Button::new("project", "project"))
                        .child(
                            ui::h_flex()
                                .min_w_0()
                                .child(ui::Button::new("graph", "graph"))
                                .child(self.status.clone()),
                        ),
                )
                .child(
                    gpui::div()
                        .debug_selector(|| "TEST_WINDOW_CONTROLS".into())
                        .flex_none()
                        .w(gpui::px(100.))
                        .h_full(),
                )
    ...
    async fn test_recent_commits_dismiss_on_outside_click(cx: &mut gpui::TestAppContext) {
        let (_, status, _workspace) = setup(18, cx).await;
        let window = *cx.windows().first().unwrap();
        let cx = &mut gpui::VisualTestContext::from_window(window, cx);
        cx.refresh().unwrap();
        cx.run_until_parked();
    ...
    #[gpui::test]
    async fn test_recent_commits_title_bar_layout_and_keyboard(cx: &mut gpui::TestAppContext) {
        let (_, status, workspace) = setup(18, cx).await;
        let window = *cx.windows().first().unwrap();
        let cx = &mut gpui::VisualTestContext::from_window(window, cx);
        let history = status.read_with(cx, |status, _| status.history.clone().unwrap());
        for font_size in [12., 20., 16.] {
            cx.update(|_, cx| {
                theme_settings::adjust_ui_font_size(cx, |_| gpui::px(font_size));
            });
            let mut previous_width = gpui::px(0.);
            let mut chrome_width = None;
            for subject in ["fix", "added tab-local split preview for markdown"] {
                history.update(cx, |history, cx| {
                    history.head.message = subject.into();
                    cx.notify();
                });
                cx.refresh().unwrap();
                cx.run_until_parked();
                let entry = cx.debug_bounds("RECENT_COMMITS_ENTRY").unwrap();
                let subject_bounds = cx.debug_bounds("RECENT_COMMITS_SUBJECT").unwrap();
                let subject_width = cx.update(|window, cx| {
                ...
                let maximum_width = cx.update(|window, _| window.rem_size() * 48.);
                assert!(entry.size.width > previous_width);
                assert!(entry.size.width < maximum_width);
                assert!((subject_bounds.size.width - subject_width).abs() <= gpui::px(1.));
                assert!(subject_bounds.right() <= entry.right());
                let current_chrome_width = entry.size.width - subject_bounds.size.width;
                if let Some(chrome_width) = chrome_width {
                    assert!((current_chrome_width - chrome_width).abs() <= gpui::px(1.));
                }
                chrome_width = Some(current_chrome_width);
                previous_width = entry.size.width;
            }
        }
        history.update(cx, |history, cx| {
            history.head.message =
                format!("{}\n\nCommit body", "很长的提交标题 ".repeat(30)).into();
            assert_eq!(history.head_sha.display_short(), "1212121");
            cx.notify();
        });
        cx.refresh().unwrap();
        cx.run_until_parked();
        let wide_entry = cx.debug_bounds("RECENT_COMMITS_ENTRY").unwrap();
        let maximum_width = cx.update(|window, _| window.rem_size() * 48.);
        assert_eq!(wide_entry.size.width, maximum_width);
        let wide_subject = cx.debug_bounds("RECENT_COMMITS_SUBJECT").unwrap();
        assert!(wide_subject.right() <= wide_entry.right());
        let wide_sha = cx.debug_bounds("RECENT_COMMITS_SHA").unwrap();
        let handle = status.read_with(cx, |status, _| status.menu_handle.clone());
        cx.update(|window, cx| handle.show(window, cx));
        cx.run_until_parked();
        let popover = cx.debug_bounds("RECENT_COMMITS_POPOVER").unwrap();
        assert_eq!(popover.left(), wide_entry.left());
        assert_eq!(popover.top(), wide_entry.bottom() + gpui::px(4.));
        cx.update(|_, cx| handle.hide(cx));
        cx.run_until_parked();
        cx.simulate_resize(gpui::size(gpui::px(400.), gpui::px(600.)));
        cx.refresh().unwrap();
        cx.run_until_parked();
        let entry = cx.debug_bounds("RECENT_COMMITS_ENTRY").unwrap();
        let sha = cx.debug_bounds("RECENT_COMMITS_SHA").unwrap();
        let controls = cx.debug_bounds("TEST_WINDOW_CONTROLS").unwrap();
        assert!(entry.size.width > gpui::px(0.));
        assert!(entry.right() <= controls.left());
        assert!(controls.right() <= gpui::px(400.));
        assert_eq!(sha.size.width, wide_sha.size.width);
        assert!(sha.right() <= entry.right());
        let narrow_subject = cx.debug_bounds("RECENT_COMMITS_SUBJECT").unwrap();
        assert!(narrow_subject.size.width < wide_subject.size.width);
        assert!(narrow_subject.right() <= entry.right());
        ...
        assert!(handle.is_deployed());
        let popover = cx.debug_bounds("RECENT_COMMITS_POPOVER").unwrap();
        assert_eq!(popover.top(), entry.bottom() + gpui::px(4.));
        assert!(popover.right() <= gpui::px(400.));
        // PopoverMenu transfers focus after two frames; tests have no platform frame loop.
        cx.update(|window, cx| {
            window.simulate_next_frame(cx);
            window.simulate_next_frame(cx);
            assert!(handle.is_focused(window, cx));
        });
        cx.simulate_keystrokes("down");
        history.read_with(cx, |history, _| assert_eq!(history.selected_index, 1));
        ...
        workspace.read_with(cx, |workspace, cx| {
            ...
            assert_eq!(
                graph.read(cx).selected_commit_sha_for_test(),
                Some(git::Oid::from_bytes(&[17; 20]).unwrap())
            );
        });
    }
