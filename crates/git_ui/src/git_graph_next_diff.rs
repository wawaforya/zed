pub(crate) struct GitGraphNextDiff {
    ...
    _load_task: Option<gpui::Task<anyhow::Result<()>>>,
    load_generation: u64,
    showing_all_lines: bool,
    files: Vec<LoadedDiffFile>,
}

struct LoadedDiffFile {
    path: multi_buffer::PathKey,
    buffer: gpui::Entity<language::Buffer>,
    diff: gpui::Entity<buffer_diff::BufferDiff>,
    is_binary: bool,
}
...
impl GitGraphNextDiff {
    pub(crate) fn new(
        ...
        let editor = gpui::AppContext::new(cx, |cx| {
            ...
            editor.rhs_editor().update(cx, |editor, cx| {
                ...
                editor.set_allow_git_diff_scrollbar_markers(false, cx);
            });
            editor
        });
        ...
        Self {
            ...
            _load_task: None,
            load_generation: 0,
            showing_all_lines: false,
            files: Vec::new(),
        }
    }

    pub(crate) fn clear(&mut self, cx: &mut gpui::Context<Self>) {
        self._load_task = None;
        self.load_generation += 1;
        self.load_error = None;
        for file in self.files.drain(..) {
            self.editor.update(cx, |editor, cx| {
                editor.remove_excerpts_for_path(file.path, cx);
            });
        }
        cx.notify();
    }
    ...
    pub(crate) fn load(
        ...
        self.clear(cx);
        self.showing_all_lines = showing_all_lines;
            ...
            let result: anyhow::Result<()> = async {
                for commit_file in commit_diff.files {
                    ...
                    let path = cx.update(|_, cx| {
                        ...
                        anyhow::Ok(multi_buffer::PathKey::with_sort_prefix(1, file.path().clone()))
                    })??;
                    ...
                    this.update(cx, |this, cx| {
                        ...
                        this.files.push(LoadedDiffFile {
                            path,
                            buffer,
                            diff: buffer_diff,
                            is_binary,
                        });
                        this.set_showing_all_lines(this.showing_all_lines, cx);
                    })?;
                }
    ...
    pub(crate) fn set_showing_all_lines(
        &mut self,
        showing_all_lines: bool,
        cx: &mut gpui::Context<Self>,
    ) {
        ...
        for file in &self.files {
            ...
            let ranges = if showing_all_lines || file.is_binary || hunks.peek().is_none() {
                vec![language::Point::zero()..snapshot.max_point()]
            } else {
                hunks
                    .map(|hunk| language::OffsetRangeExt::to_point(&hunk.buffer_range, &snapshot))
                    .collect()
            };
            let context_line_count = if showing_all_lines {
                0
            } else {
            ...
            self.editor.update(cx, |editor, cx| {
                ...
                for editor in std::iter::once(editor.rhs_editor()).chain(editor.lhs_editor()) {
